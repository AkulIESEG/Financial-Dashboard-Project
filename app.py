...
def calculate_probability(final_prices, threshold):
    # Ensure final_prices is 1-dimensional (flatten if necessary)
    final_prices = final_prices.flatten() if isinstance(final_prices, np.ndarray) else np.array(final_prices).flatten()

    # Ensure threshold is a scalar (float or int)
...
stock_data = pd.read_csv('AAPL.csv')  # replace with actual stock data loading method
